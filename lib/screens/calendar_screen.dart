    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          setState(() {
            _workouts.forEach((date, exercises) {
          ...
          await _fetchWorkouts(); // Full refresh to ensure consistency
        } else {
...
class GymExercise {
  ...
  factory GymExercise.fromJson(Map<String, dynamic> json) {
    return GymExercise(
      workoutId: int.parse(json['workout_id'].toString()),
      userId: int.parse(json['user_id'].toString()),
      date: DateTime.parse(json['date']),
      muscleGroup: json['muscle_group'] ?? 'Unknown',
      exerciseName: json['exercise_name'] ?? '',
      sets: int.parse(json['sets'].toString()),
      reps: int.parse(json['reps'].toString()),
      isCompleted: (json['status'] ?? 'planned').toString().toLowerCase() == 'complete',
