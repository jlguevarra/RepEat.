...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> {
  ...
  Future<void> _verifyCode() async {
    ...
    final verifyRes = await http.post(
      Uri.parse('http://192.168.0.25/repEatApi/verify_email_code.php'),
      body: {
        'email': widget.email,
    ...
    if (verifyData['success']) {
      final registerRes = await http.post(
        Uri.parse('http://192.168.0.25/repEatApi/register_user.php'),
        body: {
          'name': widget.name,
          'email': widget.email,
  ...
  Future<void> _resendCode() async {
    ...
    final resend = await http.post(
      Uri.parse('http://192.168.0.25/repEatApi/signup.php'),
      body: {
        'name': widget.name,
        'email': widget.email,
        'password': widget.password,
